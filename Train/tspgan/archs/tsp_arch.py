...
from archs.resnetnobn import resnet45stride as resnet45
from archs.textvit_arch import TextViT as TextEncoder 
from basicsr.ops.fused_act import FusedLeakyReLU, fused_leaky_relu


stop_update_resnet = 0
    ...
    def forward(self, lq, priors64, priors32, locs): #
        ...
        if priors32 is not None:
            ...
            for b, p_32 in enumerate(priors32): #512*32*32, different batch
                p_32_256 = self.conv_32_to256(p_32.clone().detach()) #no backward to prior
                for c in range(p_32_256.size(0)): #
                    center = (locs[b][2*c] * sq_f_32.size(-1)).detach().int() ### no backward 
                    width = (locs[b][2*c+1] * sq_f_32.size(-1)).detach().int() + 2 # no backward
                    width = 16
        ...
        for b, p_64_prior in enumerate(priors64): #
            ...
            for c in range(p_64.size(0)): # for each character
                center = (locs[b][2*c] * sq_f_64.size(-1)).detach().int() # no backward
                width = (locs[b][2*c+1] * sq_f_64.size(-1)).detach().int() + 4 # no backward
                width = 32
...
class TextContextEncoderV2(nn.Module):
    def __init__(
        ...
        if stop_update_resnet:
            for param in self.resnet.parameters():
                param.requires_grad = False


        self.transformer = TextEncoder(num_classes=num_classes, dim=dim, max_length=16)
...
class TextGenerator(nn.Module):
    ...
    def forward(
        ...
        for conv1, conv2, to_rgb in zip(
            ...
            out = conv1(out, latent[:, i], noise=None) ### Upsample ->8*8
            out = conv2(out, latent[:, i + 1], noise=None) ### 1 for 2 noise[(noise_i + 2)//2]
...
class SelectText(nn.Module):
    ...
    def forward(self, labels):
        ...
        return OutEmbs #b*512*1*len(seqs)
